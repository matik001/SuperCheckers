...
void GameWindowState::init(GameWindow &window) {
    ...
    _checkers_widget = std::make_shared<CheckersWidget>(window._window.getView().getSize(), _ai_level, _are_we_white);
}
...
void GameWindowState::handle_event(const sf::Event &event) {
    _checkers_widget->handle_event(event, _window->_window);
}
...
void GameWindowState::update() {
    ...
    if(game_status != IN_PROGRESS){
        ...
        _window->change_state(std::make_shared<ResultWindowState>(game_status, std::move(background_texture)));
    }
}
